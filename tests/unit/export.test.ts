...
describe('esportazione deliberata',()=>{
  ...
  it('blocca cliente assente e cliente FIC di azienda diversa',()=>{expect(buildExportLines({...quote,client:undefined}).ok).toBe(false);expect(buildExportLines(quote,undefined,'altra').ok).toBe(false);});
