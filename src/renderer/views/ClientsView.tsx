...
import { dateIt, formatNumber } from "@/lib/format"
...
function ClientDetailsDialog({ client, details, loading, doc, onClose, onAddSite, onEditSite, requestDelete }: {
  client?: FicClientSnapshot
  details?: FicClientDetails
  loading: boolean
  doc: CashDocument
  onClose: () => void
  onAddSite: (client: FicClientSnapshot) => void
  onEditSite: (client: FicClientSnapshot, site: Site) => void
  requestDelete: (target: DeleteTarget) => void
}) {
  const sites = client ? sitesForClient(doc, client) : []
  const quotes = client ? doc.quotes.filter((quote) => quote.client?.source === "fatture_in_cloud" && quote.client.companyId === client.companyId && quote.client.clientId === client.clientId) : []
  return <Dialog open={Boolean(client)} onOpenChange={(open) => { if (!open) onClose() }}><DialogContent className="max-h-[88vh] overflow-y-auto sm:max-w-4xl"><DialogHeader><DialogTitle>{client?.displayName ?? "Dettaglio cliente"}</DialogTitle><DialogDescription>Dati correnti di Fatture in Cloud e informazioni collegate nell’archivio Cash.</DialogDescription></DialogHeader>{loading ? <div className="grid min-h-48 place-items-center text-sm text-muted-foreground"><div className="text-center"><RefreshCw className="mx-auto mb-3 size-7 animate-spin" />Recupero dettaglio cliente…</div></div> : null}{!loading && details ? <div className="space-y-6"><section><div className="mb-3 flex items-center gap-2"><h3 className="font-semibold">Informazioni cliente</h3><Badge variant="outline">FIC</Badge></div><div className="grid grid-cols-2 gap-x-6 gap-y-3 rounded-lg border p-4">{details.fields.map((field) => <div key={field.key} className="min-w-0"><div className="flex items-center gap-2 text-xs text-muted-foreground"><span>{ficFieldLabels[field.key] ?? field.key.replaceAll("_", " ")}</span><Badge variant="outline" className="px-1 py-0 text-[10px]">FIC</Badge></div><p className="mt-1 break-words text-sm">{field.value}</p></div>)}</div></section><section className="space-y-3"><div className="flex items-center justify-between"><div><h3 className="font-semibold">Sedi in Cash</h3><p className="text-sm text-muted-foreground">{sites.length} {sites.length === 1 ? "sede associata" : "sedi associate"}</p></div>{client ? <Button size="sm" variant="outline" onClick={() => onAddSite(client)}><Plus />Aggiungi sede</Button> : null}</div>{sites.length ? <Table><TableHeader><TableRow><TableHead>Nome</TableHead><TableHead>Indirizzo</TableHead><TableHead>Distanza</TableHead><TableHead className="w-12" /></TableRow></TableHeader><TableBody>{sites.map((site) => <TableRow key={site.id}><TableCell className="font-medium">{site.name}</TableCell><TableCell>{site.address}</TableCell><TableCell>{site.oneWayKm === undefined ? "Non indicata" : `${formatNumber(site.oneWayKm, 1)} km`}</TableCell><TableCell><DropdownMenu><DropdownMenuTrigger render={<Button variant="ghost" size="icon-sm" aria-label={`Azioni per ${site.name}`} />}><MoreHorizontal /></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem onClick={() => client && onEditSite(client, site)}>Modifica</DropdownMenuItem><DropdownMenuItem variant="destructive" onClick={() => requestDelete({ kind: "site", id: site.id, label: site.name })}>Elimina</DropdownMenuItem></DropdownMenuContent></DropdownMenu></TableCell></TableRow>)}</TableBody></Table> : <div className="rounded-lg border border-dashed p-5 text-center text-sm text-muted-foreground">Nessuna sede associata nell’app.</div>}</section><section className="space-y-3"><div><h3 className="font-semibold">Preventivi in Cash</h3><p className="text-sm text-muted-foreground">{quotes.length} {quotes.length === 1 ? "preventivo collegato" : "preventivi collegati"}</p></div>{quotes.length ? <Table><TableHeader><TableRow><TableHead>Data</TableHead><TableHead>Contenuto</TableHead><TableHead>Identificativo</TableHead></TableRow></TableHeader><TableBody>{quotes.map((quote) => <TableRow key={quote.id}><TableCell>{dateIt(quote.date)}</TableCell><TableCell>{quote.items.map((item) => item.name).join(", ") || "Preventivo incompleto"}</TableCell><TableCell className="font-mono text-xs text-muted-foreground">{quote.id}</TableCell></TableRow>)}</TableBody></Table> : <div className="rounded-lg border border-dashed p-5 text-center text-sm text-muted-foreground">Nessun preventivo collegato nell’app.</div>}</section></div> : null}<DialogFooter><Button variant="outline" onClick={onClose}>Chiudi</Button></DialogFooter></DialogContent></Dialog>
}
