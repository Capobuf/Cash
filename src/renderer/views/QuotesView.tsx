  ...
  return (
    <div className="space-y-4">
      ...
      <Card><CardContent className="grid grid-cols-[150px_170px_minmax(180px,1fr)_minmax(180px,1fr)_180px] gap-3 py-4">
        ...
        <Field><FieldLabel>Cliente</FieldLabel><Button className="w-full justify-start" variant="outline" onClick={() => setOverlay({ kind: "customer" })}><UserRound />{quote.client?.displayName ?? "Scegli cliente"}{quote.client ? <Badge className="ml-auto" variant="secondary">{quote.client.source === "local" ? "Locale" : "FIC"}</Badge> : null}</Button></Field>
