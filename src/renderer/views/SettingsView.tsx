  ...
  return (
    <>
      <Tabs value={tab} onValueChange={setTab} className="space-y-5">
        ...
        <TabsContent value="integrations"><div className="space-y-4"><OpenRouteServiceCard appState={appState} /><Card><CardHeader><div><CardTitle>Fatture in Cloud</CardTitle><CardDescription>Integrazione facoltativa; tutto il flusso locale resta disponibile anche se disattivata.</CardDescription></div><CardAction><Badge variant={deviceState === "Attiva" ? "default" : deviceState === "Errore collegamento" ? "destructive" : "secondary"}>{deviceState}</Badge></CardAction></CardHeader><CardContent className="space-y-5">
...
function OpenRouteServiceCard({ appState }: { appState: AppState }) {
  ...
  useEffect(() => { void window.cash.ors.hasApiKey().then((result) => { if (result.ok) setHasKey(result.value) }) }, [])
  const verify = async () => {
    setBusy(true); setOutcome(undefined)
    if (apiKey.trim()) { const saved = await window.cash.ors.setApiKey(apiKey); if (!saved.ok) { setBusy(false); appState.setError(saved.error); return } setHasKey(true); setApiKey("") }
    const result = await window.cash.ors.verify(); setBusy(false)
    setOutcome(result.ok ? { ok: true, message: "Connessione verificata con una richiesta autenticata reale." } : { ok: false, message: [result.error.message, result.error.action].filter(Boolean).join(" ") })
  }
  return <Card><CardHeader><div><CardTitle>OpenRouteService</CardTitle><CardDescription>Geocodifica e percorso stradale. La chiave resta nel Gestore credenziali locale.</CardDescription></div><CardAction><Badge variant={outcome?.ok ? "default" : hasKey ? "secondary" : "outline"}>{outcome?.ok ? "Verificata" : hasKey ? "Chiave presente" : "Da configurare"}</Badge></CardAction></CardHeader><CardContent className="space-y-4"><Field><FieldLabel htmlFor="ors-key">API key</FieldLabel><Input id="ors-key" type="password" value={apiKey} onChange={(event) => setApiKey(event.target.value)} placeholder={hasKey ? "Inserisci solo per sostituire la chiave" : "Inserisci la API key"} /><FieldDescription>Non viene registrata nei log né salvata nell’archivio condiviso.</FieldDescription></Field><Button onClick={() => void verify()} disabled={busy || (!hasKey && !apiKey.trim())}>{busy ? "Verifica…" : "Verifica connessione"}</Button>{outcome ? <Alert variant={outcome.ok ? "default" : "destructive"}><AlertDescription>{outcome.message}</AlertDescription></Alert> : null}</CardContent></Card>
}
