...
const pathLabels: Record<string, string> = {
  ...
  annualInsurance: 'assicurazione annua', annualTax: 'bollo annuo', annualMaintenance: 'manutenzione annua',
  monthlyAmount: 'importo mensile', revenueTarget: 'fatturato obiettivo', specificAnnualExpenses: 'spese specifiche annue',
  contributionCeiling: 'massimale contributivo', ordinaryThreshold: 'soglia ordinaria', cessationThreshold: 'soglia di cessazione',
  year: 'anno', fiscal: 'parametri fiscali', capacity: 'capacità lavorativa',
};

const issueLocation = (path: PropertyKey[]): string => path.map((part) => typeof part === 'number' ? `${part + 1}` : pathLabels[String(part)] ?? String(part)).join(' · ');
...
export function validationErrorFromIssues(issues: readonly ValidationIssue[]): CashError {
  const details = issues.slice(0, 8).map((issue) => {
    ...
    const message = issue.code === 'too_small' && ['name', 'displayName', 'category', 'description'].includes(String(issue.path.at(-1))) ? 'campo obbligatorio' : issue.message;
...
const boundedDecimal = (maxDecimals: number, min?: string, max?: string, exclusiveMin = false) => decimal.superRefine((value, ctx) => {
  ...
  if (parsed.decimalPlaces() > maxDecimals) ctx.addIssue({ code: 'custom', message: `massimo ${maxDecimals} decimali` });
...
const clientSnapshotSchema = ficClientRefSchema.extend({ vatNumber: z.string().optional() });
const coordinatesSchema = z.object({ longitude: coordinate, latitude: coordinate }).superRefine((value, ctx) => {
  if (new Decimal(value.longitude).lt(-180) || new Decimal(value.longitude).gt(180)) ctx.addIssue({ code: 'custom', path: ['longitude'], message: 'longitudine fuori intervallo' });
  if (new Decimal(value.latitude).lt(-90) || new Decimal(value.latitude).gt(90)) ctx.addIssue({ code: 'custom', path: ['latitude'], message: 'latitudine fuori intervallo' });
});
const resolvedLocationSchema = z.object({ coordinates: coordinatesSchema, inputKind: z.enum(['address', 'coordinates']), inputValue: z.string().min(1) });
const siteSnapshotSchema = z.object({ sourceId: uuid, name: z.string().min(1), address: z.string().min(1).optional(), coordinates: coordinatesSchema.optional() });
...
const quoteTravelSchema = z.object({ ...baseSub, kind: z.literal('travel'), departure: siteSnapshotSchema.optional(), destination: siteSnapshotSchema.optional(), vehicleId: uuid.optional(), vehicleName: z.string().min(1).optional(), roundTrip: z.boolean(), occurrences: z.number().int().positive(), distanceKmPerOccurrence: distance.optional(), travelMinutesPerOccurrence: z.number().int().positive().optional(), distanceSource: z.enum(['route', 'manual']).optional(), durationSource: z.enum(['route', 'manual']).optional(), totalMinutes: z.number().int().positive().optional(), totalDistanceKm: distance.optional(), vehicleCostPerKm: perKmValue.optional(), totalCost: moneyInput.optional(), fuelEvidence: fuelEvidenceSchema.optional() }).superRefine((travel, ctx) => {
  if ((travel.distanceKmPerOccurrence === undefined) !== (travel.totalDistanceKm === undefined)) ctx.addIssue({ code: 'custom', message: 'distanza per occorrenza e totale devono essere entrambe presenti' });
  if ((travel.travelMinutesPerOccurrence === undefined) !== (travel.totalMinutes === undefined)) ctx.addIssue({ code: 'custom', message: 'tempo per occorrenza e totale devono essere entrambi presenti' });
  if (travel.totalCost !== undefined && (travel.vehicleCostPerKm === undefined || travel.totalDistanceKm === undefined)) ctx.addIssue({ code: 'custom', message: 'il costo richiede distanza e costo chilometrico' });
});
export const quoteSubItemSchema = z.discriminatedUnion('kind', [z.object({ ...baseSub, kind: z.literal('time'), minutes: z.number().int().positive() }), z.object({ ...baseSub, kind: z.literal('expense'), amount: moneyInput }), quoteTravelSchema]);

const reusableTravelFields = { kind: z.literal('travel'), description: z.string().min(1), roundTrip: z.boolean(), occurrences: z.number().int().positive(), distanceKmPerOccurrence: distance.optional(), travelMinutesPerOccurrence: z.number().int().positive().optional() };
const reusableSubItemSchema = z.discriminatedUnion('kind', [z.object({ ...entity, kind: z.literal('time'), description: z.string().min(1), minutes: z.number().int().positive() }), z.object({ ...entity, kind: z.literal('expense'), description: z.string().min(1), amount: moneyInput }), z.object({ ...entity, ...reusableTravelFields })]);
const definitionSchema = z.discriminatedUnion('kind', [z.object({ kind: z.literal('time'), description: z.string().min(1), minutes: z.number().int().positive() }), z.object({ kind: z.literal('expense'), description: z.string().min(1), amount: moneyInput }), z.object(reusableTravelFields)]);
const variantOptionSchema = z.object({ ...entity, name: z.string().min(1), subItems: z.array(definitionSchema) });
const variantGroupSchema = z.object({ ...entity, name: z.string().min(1), options: z.array(variantOptionSchema).min(1), defaultOptionId: uuid.optional() }).superRefine((group, ctx) => {
  const names = new Set<string>();
  for (const [index, option] of group.options.entries()) { const key = option.name.trim().toLocaleLowerCase('it'); if (names.has(key)) ctx.addIssue({ code: 'custom', path: ['options', index, 'name'], message: 'nome opzione duplicato nel gruppo' }); names.add(key); }
  if (group.defaultOptionId && !group.options.some((option) => option.id === group.defaultOptionId)) ctx.addIssue({ code: 'custom', path: ['defaultOptionId'], message: 'opzione predefinita non appartenente al gruppo' });
});
...
const fiscalSchema = z.object({ atecoCode: z.string(), profitabilityCoefficient: percentage, contributionRate: percentage, contributionCeiling: moneyInput, activityPhase: z.enum(['reduced_eligible', 'ordinary']), reducedEligibilityConfirmed: z.boolean(), ordinaryApplicabilityConfirmed: z.boolean(), reducedSubstituteTaxRate: percentage, ordinarySubstituteTaxRate: percentage, ordinaryThreshold: moneyInput, cessationThreshold: moneyInput });
const holidaySchema = z.discriminatedUnion('kind', [z.object({ kind: z.literal('recurring'), name: z.string().min(1), month: z.number().int().min(1).max(12), day: z.number().int().min(1).max(31) }), z.object({ kind: z.literal('specific'), name: z.string().min(1), date: z.string().date() })]);
const capacitySchema = z.object({ hoursPerDay: boundedDecimal(4, '0', '24'), vacationDays: z.number().int().nonnegative(), unplannedDays: z.number().int().nonnegative(), clientTimePercentage: percentage, localHolidays: z.array(holidaySchema) });
export const profileSchema = z.object({ ...entity, year: z.number().int().min(2000).max(2200), revision: z.number().int().positive(), confirmed: z.boolean(), revenueTarget: moneyInput, specificAnnualExpenses: moneyInput, fiscal: fiscalSchema, capacity: capacitySchema }).superRefine((profile, ctx) => {
  if (new Decimal(profile.fiscal.cessationThreshold).lt(profile.fiscal.ordinaryThreshold)) ctx.addIssue({ code: 'custom', path: ['fiscal', 'cessationThreshold'], message: 'la soglia di cessazione deve essere almeno pari alla soglia ordinaria' });
  ...
  if (profile.fiscal.activityPhase === 'reduced_eligible' && !profile.fiscal.reducedEligibilityConfirmed) ctx.addIssue({ code: 'custom', path: ['fiscal', 'reducedEligibilityConfirmed'], message: 'conferma aliquota ridotta obbligatoria' });
  ...
  if (profile.fiscal.activityPhase === 'ordinary' && revenue.gt(ordinary) && revenue.lte(cessation) && !profile.fiscal.ordinaryApplicabilityConfirmed) ctx.addIssue({ code: 'custom', path: ['fiscal', 'ordinaryApplicabilityConfirmed'], message: 'conferma applicabilità oltre soglia obbligatoria' });
});
...
export const cashDocumentSchema: z.ZodType<CashDocument> = z.object({
  ...
  if (document.settings.defaultDepartureSiteId && !document.sites.some((site) => site.id === document.settings.defaultDepartureSiteId)) ctx.addIssue({ code: 'custom', path: ['settings', 'defaultDepartureSiteId'], message: 'Sede di partenza predefinita inesistente' });
  if (document.settings.defaultVehicleId && !document.vehicles.some((vehicle) => vehicle.id === document.settings.defaultVehicleId)) ctx.addIssue({ code: 'custom', path: ['settings', 'defaultVehicleId'], message: 'Veicolo predefinito inesistente' });
});

export const documentHeaderSchema = z.object({ schemaVersion: z.number().int().positive(), documentId: uuid, revision: z.number().int().positive(), createdAt: iso, updatedAt: iso });
export function parseDocument(input: unknown): CashDocument { return cashDocumentSchema.parse(input); }
