...
| Recuperi di Spese specifiche previsti | Quota annua del Fatturato obiettivo che si prevede serva a recuperare Spese direttamente legate ai lavori; valore non negativo, inizialmente 0. |
| Fatturato da generare con il tempo | Fatturato obiettivo meno Recuperi di Spese specifiche previsti. |
...
> **Fatturato da generare con il tempo:** Fatturato obiettivo - Recuperi di Spese specifiche previsti
...
La prima versione è costruita intorno a un solo caso: professionista informatico in regime forfettario, oltre i primi cinque anni di attività, iscritto in via esclusiva alla Gestione Separata INPS e non pensionato. Non è richiesto un motore fiscale generico. La qualifica generica di “professionista informatico” non determina però da sola il coefficiente di redditività: il profilo deve includere il codice ATECO effettivo confermato dall'utente.

Il profilo fiscale è annuale, datato e interamente modificabile. Il preset iniziale 2026 contiene i valori seguenti; prima della prima proiezione fiscale l'utente deve confermarli esplicitamente. Cash non li aggiorna automaticamente negli anni successivi.

| **Parametro** | **Preset 2026** | **Uso** |
| --- | --- | --- |
| Anno fiscale | 2026 | Identifica la validità temporale del profilo. |
| Codice attività ATECO 2025 | 62.20.10 | Proposta iniziale per consulenza informatica; deve essere sostituita se non coincide con l'attività effettiva. |
| Coefficiente di redditività | 67% | Valore associato al gruppo delle attività informatiche della divisione 62; deve essere coerente con il codice ATECO confermato. |
| Aliquota contributiva | 26,07% | Stima i contributi per il profilo previdenziale previsto. |
| Massimale contributivo | € 122.295,00 | Limita la base soggetta a contribuzione. |
| Aliquota imposta sostitutiva | 15% | Stima l'imposta per attività oltre il periodo agevolato iniziale. |
| Soglia ordinaria di permanenza | € 85.000,00 | Produce un avviso se superata dal Fatturato obiettivo. |
| Soglia di cessazione nell'anno | € 100.000,00 | Oltre questa soglia il preset forfettario non può produrre una proiezione fiscale valida. |
...
> **Contributi stimati:** Base contributiva × Aliquota contributiva

> **Base imposta:** max(0, Reddito forfettario - Contributi stimati)

> **Imposta stimata:** Base imposta × Aliquota imposta sostitutiva

Il minimale INPS è informativo ai fini dell'accredito contributivo e non viene trasformato in un contributo minimo dovuto. Se il profilo reale dell'utente non coincide con quello descritto, il preset deve essere modificato prima dell'uso.

Se il Fatturato obiettivo supera la soglia ordinaria di permanenza, Cash mostra un avviso forte e richiede una conferma specifica dell'applicabilità del regime nell'anno. Se supera la soglia di cessazione nell'anno, Cash non mostra contributi, imposta o netto fiscale calcolati con il preset forfettario: la simulazione di un regime diverso è fuori scope. Il Fatturato obiettivo può comunque essere salvato e usato per la pianificazione non fiscale.

Il passaggio a un nuovo anno non copia silenziosamente aliquote o massimali. Cash crea un nuovo profilo annuale a partire dall'ultimo solo dopo un'azione esplicita, lo marca “da verificare” e impedisce i calcoli fiscali finché l'utente non lo conferma.
...
Il valore medio da generare non è una tariffa oraria obbligatoria. Indica quanto fatturato, al netto della quota annua destinata a recuperare Spese specifiche, dovrebbe essere generato mediamente per ogni ora della capacità dedicata ai clienti affinché l'obiettivo annuale sia raggiungibile.
...
Il cliente è facoltativo durante il salvataggio e i calcoli interni del preventivo, ma obbligatorio per l'esportazione. Quando viene selezionato, Cash copia nel preventivo identificativo dell'azienda Fatture in Cloud, identificativo cliente, ragione sociale e partita IVA. Modifiche successive in Fatture in Cloud non modificano i preventivi già esistenti.
...
| Cliente associato | Opzionale. Riferimento minimo composto da azienda Fatture in Cloud, identificativo cliente e denominazione leggibile; una Sede può esistere senza cliente. |
...
La sede/laboratorio di riferimento è implicita nel significato della distanza e non viene modellata come partenza della Trasferta. Il riferimento cliente salvato su una Sede serve soltanto a ricerca e proposta: non è una cache dell'anagrafica e non sostituisce una ricerca live quando l'operazione la richiede.
...
- Se selezionato, azienda e identificativo cliente Fatture in Cloud, ragione sociale e partita IVA.
...
- Fatturato obiettivo, Recuperi di Spese specifiche previsti e Fatturato da generare con il tempo da cui deriva il valore medio.
...
Per l'MVP Cash usa l'autenticazione manuale prevista da Fatture in Cloud, coerente con un'applicazione privata monoutente. Cash distribuisce o configura il Client ID della propria applicazione privata e non conserva alcun Client Secret. L'utente genera una volta un token con i permessi minimi necessari e lo incolla in Cash. Sono richiesti almeno `entity.clients:r` per leggere i clienti, `products:r` per cercare e verificare il prodotto e `issued_documents.quotes:a` per creare preventivi; eventuali permessi ulteriori devono essere giustificati da una funzione effettivamente implementata. Il collegamento è facoltativo finché l'utente non richiede ricerca clienti, verifica del prodotto o esportazione.
...
Cash interroga in tempo reale l'azienda configurata, permette di cercare e selezionare un cliente e copia nel preventivo identificativo dell'azienda, identificativo cliente Fatture in Cloud, ragione sociale e partita IVA. Le Sedi non vengono importate.

Cash non conserva una cache del catalogo clienti come sorgente alternativa. Se il servizio non è raggiungibile, la ricerca live fallisce; i clienti già copiati nei preventivi rimangono disponibili nei rispettivi snapshot.
...
In caso di successo Cash salva nel preventivo l'identificativo del documento remoto, l'azienda, la data/ora e le righe inviate. Se la richiesta ha esito di rete incerto dopo l'invio, Cash non ripete automaticamente l'operazione: mostra “esito esportazione da verificare” e chiede all'utente di controllare Fatture in Cloud prima di un eventuale nuovo tentativo. Se il preventivo è già stato esportato, una nuova esportazione resta possibile ma richiede una conferma esplicita per evitare duplicati.
...
**2.** Confermare o modificare il profilo fiscale annuale.

**3.** Inserire il Fatturato obiettivo annuo, l'eventuale quota prevista di recupero delle Spese specifiche e verificare netto fiscale stimato, disponibile stimato e Fatturato da generare con il tempo.
...
**9.** Creare le Sedi riutilizzabili necessarie e, quando serve, indicarne cliente associato e distanza di sola andata dalla sede/laboratorio di riferimento.

**10.** Se si vogliono usare le funzioni live, collegare separatamente la postazione a Fatture in Cloud con il token manuale e selezionare azienda e prodotto “Consulenza”. La configurazione può essere rinviata senza bloccare le funzioni locali.
...
**1.** Se disponibile e necessario, selezionare il cliente da Fatture in Cloud; Cash copia azienda, identificativo cliente, ragione sociale e partita IVA nel preventivo. La scelta può essere rinviata, ma deve essere completata prima dell'esportazione.
...
**4.** Se la postazione non è ancora collegata a Fatture in Cloud, configurare localmente il token; nessun token viene trasferito da Drive.
...
| --- | --- |
| Profilo economico | Anno, fatturato obiettivo, recuperi annui previsti di Spese specifiche, parametri fiscali confermati, disponibilità lavorativa, festività locali e velocità media di trasferta. |
...
| Cliente | Riferimento alla fonte Fatture in Cloud composto da azienda e identificativo cliente; nel preventivo vengono copiati anche ragione sociale e partita IVA. |
...
| Preventivo | Eventuale snapshot cliente, eventuale Sede principale, data, insieme di voci, eventuale provvigione e valori economici usati. Non ha stato commerciale. |
...
| Riferimento esportazione | Azienda, identificativo documento Fatture in Cloud, data/ora, righe inviate ed eventuale esito incerto; non è uno stato commerciale. |
...
| Recuperi di Spese specifiche previsti | Importo annuo non negativo e inferiore al Fatturato obiettivo; la differenza deve essere positiva per calcolare il valore medio da generare. |
...
| Profilo fiscale | Anno, codice ATECO, coefficiente coerente, aliquote, massimale e soglie obbligatori; profilo confermato prima delle proiezioni fiscali. Oltre la soglia di cessazione il preset forfettario non calcola indicatori fiscali. |
...
| Template | Deve contenere almeno una voce; ogni voce deve contenere almeno una sottovoce Tempo o Trasferta. I riferimenti contestuali esclusi dalle Trasferte vengono richiesti dopo l'inserimento nel preventivo. |
...
| Fatture in Cloud | Token, azienda, cliente, prodotto e permessi richiesti devono essere validi per la specifica operazione live; non sono obbligatori per l'uso locale. Cliente e prodotto devono appartenere all'azienda configurata. |
...
La modifica di profili, costi, veicoli, Sedi, sottovoci di catalogo o template vale soltanto per utilizzi futuri. I preventivi esistenti restano invariati finché l'utente non modifica direttamente il preventivo o esegue “Aggiorna con valori correnti”.
...
Il client desktop è necessario per soddisfare contemporaneamente accesso affidabile al file dati, salvataggi atomici, custodia delle credenziali nel sistema operativo e chiamate alle API Fatture in Cloud. Una pagina aperta direttamente con `file://` non è il runtime canonico dell'MVP: l'API Fatture in Cloud non espone gli header CORS necessari alle chiamate dal browser e non è lecito introdurre un proxy server.
...
L'host nativo del client esegue richieste HTTPS soltanto verso gli endpoint ufficiali necessari di ISTAT, MIMIT e Fatture in Cloud e restituisce all'interfaccia dati strutturati validati. La WebView non chiama direttamente tali API e non riceve accesso generico alla rete o al filesystem. Cash non usa proxy, servizi intermedi o API proprietarie di Cash. Google Drive viene usato tramite il normale filesystem sincronizzato da Drive for Desktop; Cash non richiede Google Drive API né OAuth Google.

Senza connessione internet l'utente può aprire e modificare dati locali, catalogo e preventivi già salvati. Sono invece bloccate, con errore esplicito, le sole operazioni che richiedono dati live: acquisizione di nuovi indici FOI o prezzi carburante, ricerca clienti, verifica prodotto ed esportazione Fatture in Cloud. Gli snapshot esistenti restano consultabili e ricalcolabili con i propri dati storici.
...
- costi aziendali, veicoli e Sedi;
- catalogo e template;
- preventivi e relativi snapshot;
- riferimenti non segreti alle esportazioni.

Importi, aliquote, coefficienti, distanze e consumi sono serializzati come stringhe decimali con punto come separatore e unità definita dallo schema. Durate e occorrenze sono interi. Il file non contiene token, password, cache clienti o copie nascoste di dati live.
...
Se uno dei passaggi fallisce, Cash non considera salvata la modifica e mantiene in memoria la versione locale non salvata. Non tronca né sostituisce il file principale con contenuto parziale. La chiusura dell'applicazione con modifiche non salvate richiede una scelta esplicita fra riprovare, salvare una copia di recupero in un percorso scelto dall'utente o annullare le modifiche e chiudere.
...
Le impostazioni esistono solo per preferenze o dati realmente variabili. Sono divise fra condivise nel file dati e locali alla postazione.

| **Impostazione condivisa** | **Regola** |
| --- | --- |
| Profilo economico/fiscale per anno | Obbligatorio e confermato; contiene obiettivo, codice ATECO, coefficiente, aliquote, soglie fiscali e disponibilità. |
| Recuperi di Spese specifiche previsti | Importo annuo non negativo e inferiore al Fatturato obiettivo; valore iniziale 0. |
| Ore lavorative al giorno | Maggiori di zero. |
...
| Azienda Fatture in Cloud | Identificativo non segreto dell'azienda scelta; facoltativo finché non viene richiesta una funzione live. |
| Prodotto “Consulenza” | Identificativo non segreto; facoltativo fino all'esportazione e verificato live prima dell'invio. |

| **Impostazione locale** | **Regola** |
| --- | --- |
| Token Fatture in Cloud | Nel gestore credenziali del sistema operativo; mai in Drive. |
...
I seguenti sono dati di configurazione che l'utente deve fornire quando pertinenti, non decisioni di prodotto: conferma del profilo fiscale annuale e del codice ATECO, festività locale, regione/provincia autonoma per il carburante, token e azienda Fatture in Cloud, prodotto “Consulenza” e percorso del file dati.

## 29. Criteri di accettazione dell'MVP

L'MVP è funzionalmente coerente con questa specifica quando consente almeno quanto segue:

**1.** Impostare Fatturato obiettivo e Recuperi di Spese specifiche previsti, quindi visualizzare immediatamente Fatturato da generare con il tempo, netto fiscale stimato, costi aziendali annui e disponibile stimato.

**2.** Proporre il preset fiscale 2026 con ATECO 62.20.10 e coefficiente 67%, richiederne la conferma, gestire gli avvisi sulle soglie del regime e bloccare le proiezioni fiscali di un nuovo anno finché il relativo profilo non è confermato.
...
**14.** Permettere di salvare un preventivo senza cliente; quando viene selezionato da Fatture in Cloud, salvarne azienda, identificativo, ragione sociale e partita IVA come snapshot e renderlo obbligatorio solo per l'esportazione.
...
**38.** Mostrare l'anteprima, raggruppare/esportare le voci verso Fatture in Cloud usando cliente e prodotto “Consulenza” verificati nella stessa azienda, descrizioni modificabili e quantità 1, senza esportare automaticamente sottovoci, Spese o provvigione.
...
**42.** Conservare tutti i dati funzionali in un unico file JSON UTF-8 versionato, senza credenziali o cache clienti.
...
**48.** Conservare il token Fatture in Cloud solo nel gestore credenziali locale del sistema operativo e richiedere una configurazione separata su ogni postazione.

**49.** Gestire un esito di rete incerto dell'esportazione senza ritentare automaticamente e avvertire prima di riesportare un preventivo già esportato.
...
**53.** Consentire l'uso offline delle funzioni che non richiedono fonti live e bloccare selettivamente quelle che richiedono ISTAT, MIMIT o Fatture in Cloud.

**54.** Non includere HTMX nell'MVP e non usarlo per simulare un backend, caricare file locali o aggirare i vincoli CORS.

## 30. Fonti normative e tecniche

Verifica effettuata il 7 settembre 2026. Le fonti sono riferimenti di progettazione; il file dati salva sempre i valori effettivamente usati nei calcoli.
...
→ Fatturato obiettivo - recuperi annui previsti di Spese specifiche → fatturato da generare con il tempo
